...
async function main() {
  ...
  try {
    ...
    for (const { employee_id } of employees) {
      ...
      const exists = await conn.query('SELECT id FROM conversations WHERE id = ? LIMIT 1', [convId]);
      if (exists.length) report.conversationsExisted++;
      else {
        await conn.query('INSERT INTO conversations (id, type, employee_id, group_id, title, version) VALUES (?, "single", ?, NULL, ?, 1)', [convId, employee_id, title]);
      ...
      for (const m of msgs) {
        const r = await conn.query('INSERT IGNORE INTO conversation_messages (id, conversation_id, sender, sender_name, text, tokens) VALUES (?, ?, ?, ?, ?, ?)', [m.id, convId, m.sender === 'me' ? 'me' : 'employee', '', m.text, Number(m.tokens || 0)]);
    ...
    for (const { group_id } of groups) {
      ...
      if (exists.length) report.conversationsExisted++;
        ...
        await conn.query('INSERT INTO conversations (id, type, employee_id, group_id, title, version) VALUES (?, "group", NULL, ?, ?, 1)', [convId, group_id, title]);
        report.conversationsCreated++;
      }
      ...
      for (const m of msgs) {
        const r = await conn.query('INSERT IGNORE INTO conversation_messages (id, conversation_id, sender, sender_name, text, tokens) VALUES (?, ?, ?, ?, ?, ?)', [m.id, convId, m.sender === 'me' ? 'me' : 'employee', m.sender_name || '', m.text, Number(m.tokens || 0)]);
    ...
    for (const l of links) {
      ...
      if (!convId) continue;
      const r = await conn.query('INSERT IGNORE INTO conversation_message_attachments (message_id, attachment_id, sort_order) VALUES (?, ?, ?)', [l.message_id, l.attachment_id, Number(l.sort_order || 0)]);
