...
export async function PUT(request: Request) {
  try {
    ...
    try {
      ...
      await connection.query('DELETE FROM employee_profiles');
      for (const [employeeId, p] of Object.entries(profiles)) {
        if (!p?.summary) continue;
        await connection.query(
          'INSERT INTO employee_profiles (employee_id, summary, traits, expertise, strengths, weaknesses, best_for, skills, nationality, age, keywords, not_good_at, career) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
          [employeeId, p.summary, JSON.stringify(p.traits || []), p.expertise || '', JSON.stringify(p.strengths || []), JSON.stringify(p.weaknesses || []), JSON.stringify(p.bestFor || []), JSON.stringify(p.skills || []), p.nationality || '', p.age === '' || p.age === undefined || p.age === null ? null : Number(p.age), JSON.stringify(p.keywords || []), JSON.stringify(p.notGoodAt || []), JSON.stringify(p.career || [])]
        );
      }
