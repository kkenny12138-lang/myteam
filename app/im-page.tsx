...
type Message = { id: string; sender: 'me' | 'employee'; text: string; time: string; tokens?: number; trace?: RunTrace; tracePending?: boolean };
...
function sortGroupMessages(list: GroupMessage[]): GroupMessage[] {
  const minutes = (time: string) => { const match = time.match(/^(\d{1,2}):(\d{2})$/); return match ? Number(match[1]) * 60 + Number(match[2]) : Number.MAX_SAFE_INTEGER; };
  return list.map((message, index) => ({ message, index })).sort((a, b) => minutes(a.message.time) - minutes(b.message.time) || a.index - b.index).map(({ message }) => message);
}
  ...
  const send = async () => {
    ...
    try {
      if (isDispatcher) {
        ...
        const activeHistory = history.slice(contextStarts[selected.id] || 0);
        const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: modelFor(selected.id), mode: answerMode, employeeId: selected.id, messages: activeHistory.map(({ sender, text: content }) => ({ sender, text: content })) }) });
