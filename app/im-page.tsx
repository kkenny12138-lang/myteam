...
type ChatModel = 'kimi' | 'deepseek' | 'openai';
type AnswerMode = 'fast' | 'deep';

const modelLabel = (model: ChatModel): string => model === 'kimi' ? 'Kimi' : model === 'deepseek' ? 'DS V4' : 'GPT';
...
type Tenant = { tenantId: string; slug: string; name: string; role: string };
type GroupSummary = { summary: string; sourceMessageCount: number; updatedAt?: string | null };
type LongTermMemory = { id: string; agentId: string; kind: 'long_term' | 'preference' | 'task_context' | 'summary'; content: string; createdAt?: string };
type MemorySuggestion = { action: 'add' | 'delete'; kind: LongTermMemory['kind']; content: string; memoryId?: string };
...
function useModalA11y(onClose: () => void) {
  ...
  useEffect(() => {
    ...
    if (node) {
      ...
      const first = node.querySelector<HTMLElement>('input, textarea, select, button, [href]');
      (first ?? node).focus();
    }
...
export default function IMPage() {
  ...
  const [memorySuggestionBusy, setMemorySuggestionBusy] = useState(false);
  const [tenants, setTenants] = useState<Tenant[]>([]);
  const [currentTenant, setCurrentTenant] = useState<Tenant | null>(null);
  const [groupContextStarts, setGroupContextStarts] = useState<Record<string, number>>({});
  ...
  const [groupSummaryBusy, setGroupSummaryBusy] = useState(false);
  ...
  useEffect(() => { queueMicrotask(() => { try { const e = localStorage.getItem('myteam-employees'); const m = localStorage.getItem('myteam-messages'); const model = localStorage.getItem('myteam-chat-model'); const mode = localStorage.getItem('myteam-answer-mode'); const starts = localStorage.getItem('myteam-context-starts'); const em = localStorage.getItem('myteam-employee-models'); const p = localStorage.getItem('myteam-profiles'); if (e) setEmployees(JSON.parse(e)); if (m) setMessages(JSON.parse(m)); if (starts) setContextStarts(JSON.parse(starts)); if (model === 'kimi' || model === 'deepseek' || model === 'openai') setChatModel(model); if (mode === 'fast' || mode === 'deep') setAnswerMode(mode); if (em) setEmployeeModels(JSON.parse(em)); if (p) setProfiles(JSON.parse(p)); } catch {} setHydrated(true); }); }, []);
  useEffect(() => { void (async () => { try { const res = await fetch('/api/auth/me'); if (!res.ok) return; const data = await res.json() as { tenants?: Tenant[] }; const list = data.tenants || []; setTenants(list); const selected = localStorage.getItem('myteam-active-tenant'); setCurrentTenant(list.find((t) => t.slug === selected) || list[0] || null); } catch {} })(); }, []);
  ...
  const startNewGroupConversation = () => { if (!activeGroup || groupReplying) return; setGroupContextStarts((current) => ({ ...current, [activeGroup.id]: (groupMessages[activeGroup.id] || []).length })); setGroupDraft(''); setGroupAiError(''); };
  const openGroupSummary = async () => {
    if (!activeGroup) return;
    setShowGroupSummary(true); setGroupSummaryBusy(true); setGroupAiError('');
    try { const response = await fetch(`/api/groups/${encodeURIComponent(activeGroup.id)}/summary`); const data = await readJsonResponse<{ summary?: string; sourceMessageCount?: number; updatedAt?: string | null; error?: string }>(response); if (!response.ok) throw new Error(data.error || '读取群摘要失败'); setGroupSummaries((cur) => ({ ...cur, [activeGroup.id]: { summary: data.summary || '', sourceMessageCount: data.sourceMessageCount || 0, updatedAt: data.updatedAt } })); } catch (error) { setGroupAiError(error instanceof Error ? error.message : '读取群摘要失败'); } finally { setGroupSummaryBusy(false); }
  };
  const refreshGroupSummary = async () => {
    if (!activeGroup) return;
    setGroupSummaryBusy(true); setGroupAiError('');
    try { const response = await fetch(`/api/groups/${encodeURIComponent(activeGroup.id)}/summary`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: chatModel }) }); const data = await readJsonResponse<{ summary?: string; sourceMessageCount?: number; updatedAt?: string | null; error?: string }>(response); if (!response.ok) throw new Error(data.error || '生成群摘要失败'); setGroupSummaries((cur) => ({ ...cur, [activeGroup.id]: { summary: data.summary || '', sourceMessageCount: data.sourceMessageCount || 0, updatedAt: data.updatedAt } })); } catch (error) { setGroupAiError(error instanceof Error ? error.message : '生成群摘要失败'); } finally { setGroupSummaryBusy(false); }
  };
  const suggestMemories = async (employeeId: string, thread: Message[]) => {
    if (memorySuggestionBusy) return;
    setMemorySuggestionBusy(true);
    try {
      const response = await fetch('/api/memories/suggest', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ agentId: `emp_${employeeId}`, model: modelFor(employeeId), messages: thread.slice(-12).map((m) => ({ sender: m.sender, text: m.text })) }) });
      const data = await readJsonResponse<{ suggestions?: MemorySuggestion[] }>(response);
      if (response.ok && data.suggestions?.length) setMemorySuggestions(data.suggestions);
    } catch { /* 记忆建议失败不影响聊天 */ } finally { setMemorySuggestionBusy(false); }
  };
  const applyMemorySuggestion = async (suggestion: MemorySuggestion) => {
    if (!selected) return;
    const url = suggestion.action === 'delete' ? `/api/memories?id=${encodeURIComponent(suggestion.memoryId || '')}` : '/api/memories';
    const response = await fetch(url, suggestion.action === 'delete' ? { method: 'DELETE' } : { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ agentId: `emp_${selected.id}`, kind: suggestion.kind, content: suggestion.content, metadata: { source: 'confirmed_suggestion' } }) });
    if (!response.ok) throw new Error('保存记忆失败');
    setMemorySuggestions((cur) => cur.filter((item) => item !== suggestion));
  };
  const send = async () => {
    ...
    try {
      if (isDispatcher) {
        ...
        const completedThread = [...history, reply];
        setMessages((current) => ({ ...current, [selected.id]: completedThread }));
        void suggestMemories(selected.id, completedThread);
      }
    } catch (error) {
  ...
  const sendGroup = async () => {
    ...
    const previousThread = groupMessages[activeGroup.id] || [];
    const contextStart = groupContextStarts[activeGroup.id] || 0;
    const history = [...previousThread.slice(contextStart), userMsg];
    const previousContext = previousThread.slice(0, contextStart);
    setGroupMessages((cur) => ({ ...cur, [activeGroup.id]: [...previousThread, userMsg] })); setGroupDraft(''); setGroupDraftAttachments([]); setGroupReplying(true); setGroupAiError('');
    ...
    try {
      ...
      setGroupMessages((cur) => ({ ...cur, [activeGroup.id]: [...previousContext, ...thread] }));
      ...
      setGroupMessages((cur) => ({ ...cur, [activeGroup.id]: [...previousContext, ...completedThread] }));
  ...
  return <main className="im-shell">
    ...
    {showTenantSwitcher && <TenantSwitcherModal tenants={tenants} currentTenant={currentTenant} onClose={() => setShowTenantSwitcher(false)} />}
    {showGroupSummary && activeGroup && <GroupSummaryModal group={activeGroup} summary={groupSummaries[activeGroup.id]} busy={groupSummaryBusy} onRefresh={() => void refreshGroupSummary()} onClose={() => setShowGroupSummary(false)} />}
    {showMemoryPanel && selected && <MemoryPanel agentId={`emp_${selected.id}`} employeeName={selected.name} onClose={() => setShowMemoryPanel(false)} />}
    {memorySuggestions.length > 0 && selected && <MemorySuggestionCard employeeName={selected.name} suggestion={memorySuggestions[0]} onConfirm={() => void applyMemorySuggestion(memorySuggestions[0])} onDismiss={() => setMemorySuggestions((cur) => cur.slice(1))} />}
  </main>;
}
function TenantSwitcherModal({ tenants, currentTenant, onClose }: { tenants: Tenant[]; currentTenant: Tenant | null; onClose: () => void }) {
  const [email, setEmail] = useState(''); const [password, setPassword] = useState(''); const [name, setName] = useState(''); const [error, setError] = useState(''); const [busy, setBusy] = useState(false); const dialogRef = useModalA11y(onClose);
  const login = async () => { setBusy(true); setError(''); try { const res = await fetch('/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) }); const data = await readJsonResponse<{ error?: string }>(res); if (!res.ok) throw new Error(data.error || '登录失败'); window.location.reload(); } catch (e) { setError(e instanceof Error ? e.message : '登录失败'); } finally { setBusy(false); } };
  const choose = async (slug: string) => { setBusy(true); setError(''); try { const res = await fetch(`/api/t/${encodeURIComponent(slug)}/select`, { method: 'POST' }); const data = await readJsonResponse<{ error?: string }>(res); if (!res.ok) throw new Error(data.error || '切换失败'); localStorage.setItem('myteam-active-tenant', slug); window.location.reload(); } catch (e) { setError(e instanceof Error ? e.message : '切换失败'); } finally { setBusy(false); } };
  const create = async () => { setBusy(true); setError(''); try { const res = await fetch('/api/tenants', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name }) }); const data = await readJsonResponse<{ tenant?: Tenant; error?: string }>(res); if (!res.ok || !data.tenant) throw new Error(data.error || '创建失败'); await choose(data.tenant.slug); } catch (e) { setError(e instanceof Error ? e.message : '创建失败'); } finally { setBusy(false); } };
  const signedIn = tenants.length > 0;
  return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="tenant-modal" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="tenant-title"><header><div><span className="eyebrow">TENANT</span><h2 id="tenant-title">{signedIn ? '切换租户' : '登录后使用租户'}</h2><p>{signedIn ? '各租户的对话、群聊和摘要完全隔离。' : '使用已有账号登录后即可切换或新建租户。'}</p></div><button aria-label="关闭" onClick={onClose}>×</button></header>{signedIn ? <><div className="tenant-list">{tenants.map((tenant) => <button key={tenant.tenantId} disabled={busy || tenant.slug === currentTenant?.slug} className={tenant.slug === currentTenant?.slug ? 'active' : ''} onClick={() => void choose(tenant.slug)}><strong>{tenant.name}</strong><span>{tenant.slug === currentTenant?.slug ? '当前租户' : tenant.role}</span></button>)}</div><div className="dl-field"><label>新增租户</label><input value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：华东销售团队" /></div><footer><button className="primary-action" disabled={busy || !name.trim()} onClick={() => void create()}>{busy ? '处理中…' : '＋ 新增租户'}</button></footer></> : <><div className="dl-field"><label>邮箱</label><input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@company.com" /></div><div className="dl-field"><label>密码</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="输入密码" onKeyDown={(e) => { if (e.key === 'Enter') void login(); }} /></div><footer><button className="primary-action" disabled={busy || !email || !password} onClick={() => void login()}>{busy ? '登录中…' : '登录'}</button></footer></>}{error && <p className="tenant-error">{error}</p>}</section></div>;
}
function GroupSummaryModal({ group, summary, busy, onRefresh, onClose }: { group: Group; summary?: GroupSummary; busy: boolean; onRefresh: () => void; onClose: () => void }) { const dialogRef = useModalA11y(onClose); return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="group-summary-modal" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="group-summary-title"><header><div><span className="eyebrow">GROUP SUMMARY</span><h2 id="group-summary-title">{group.name} · 群摘要</h2><p>新会话会引用这份摘要作为历史上下文，减少重复传输对话内容。</p></div><button aria-label="关闭" onClick={onClose}>×</button></header><div className="group-summary-body">{busy ? '正在整理群内信息…' : summary?.summary ? summary.summary : '还没有群摘要。点击下方按钮即可根据当前群消息生成。'}</div>{summary?.updatedAt && <p className="summary-meta">已覆盖 {summary.sourceMessageCount} 条消息</p>}<footer><button className="primary-action" disabled={busy} onClick={onRefresh}>{summary?.summary ? '更新群摘要' : '生成群摘要'}</button></footer></section></div>; }
function MemorySuggestionCard({ employeeName, suggestion, onConfirm, onDismiss }: { employeeName: string; suggestion: MemorySuggestion; onConfirm: () => void; onDismiss: () => void }) { return <section className="memory-suggestion" role="status"><strong>{employeeName} 建议{suggestion.action === 'delete' ? '清除' : '记住'}</strong><p>{suggestion.content}</p><div><button onClick={onDismiss}>忽略</button><button className="primary-action" onClick={onConfirm}>{suggestion.action === 'delete' ? '确认清除' : '确认记住'}</button></div></section>; }
function MemoryPanel({ agentId, employeeName, onClose }: { agentId: string; employeeName: string; onClose: () => void }) {
  const [memories, setMemories] = useState<LongTermMemory[]>([]); const [loading, setLoading] = useState(true); const [editing, setEditing] = useState<string | null>(null); const [content, setContent] = useState(''); const [error, setError] = useState(''); const dialogRef = useModalA11y(onClose);
  const load = async () => { setLoading(true); try { const res = await fetch(`/api/memories?agentId=${encodeURIComponent(agentId)}`); const data = await readJsonResponse<{ memories?: LongTermMemory[]; error?: string }>(res); if (!res.ok) throw new Error(data.error || '读取失败'); setMemories(data.memories || []); } catch (e) { setError(e instanceof Error ? e.message : '读取失败'); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, [agentId]);
  const save = async (memory: LongTermMemory) => { try { const res = await fetch('/api/memories', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: memory.id, kind: memory.kind, content }) }); if (!res.ok) throw new Error('保存失败'); setMemories((cur) => cur.map((item) => item.id === memory.id ? { ...item, content } : item)); setEditing(null); } catch (e) { setError(e instanceof Error ? e.message : '保存失败'); } };
  const remove = async (id: string) => { try { const res = await fetch(`/api/memories?id=${encodeURIComponent(id)}`, { method: 'DELETE' }); if (!res.ok) throw new Error('删除失败'); setMemories((cur) => cur.filter((item) => item.id !== id)); } catch (e) { setError(e instanceof Error ? e.message : '删除失败'); } };
  const label: Record<LongTermMemory['kind'], string> = { long_term: '长期事实', preference: '偏好', task_context: '任务上下文', summary: '摘要' };
  return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="memory-panel" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="memory-panel-title"><header><div><span className="eyebrow">LONG-TERM MEMORY</span><h2 id="memory-panel-title">{employeeName} 的长期记忆</h2><p>只保存你确认过、会影响后续协作的信息。</p></div><button aria-label="关闭" onClick={onClose}>×</button></header><div className="memory-list">{loading ? <p>正在读取…</p> : memories.length ? memories.map((memory) => <article key={memory.id}><span>{label[memory.kind]}</span>{editing === memory.id ? <><textarea value={content} onChange={(e) => setContent(e.target.value)} rows={3} /><div><button onClick={() => setEditing(null)}>取消</button><button className="primary-action" onClick={() => void save(memory)}>保存</button></div></> : <><p>{memory.content}</p><div><button onClick={() => { setEditing(memory.id); setContent(memory.content); }}>编辑</button><button className="memory-delete" onClick={() => void remove(memory.id)}>删除</button></div></>}</article>) : <p>还没有长期记忆。聊天后出现的建议经你确认后会存到这里。</p>}</div>{error && <p className="tenant-error">{error}</p>}</section></div>;
}
...
function NavButton({ active, label, icon, onClick }: { active: boolean; label: string; icon: 'chat' | 'org' | 'decision'; onClick: () => void }) { return <button className={`nav-button ${active ? 'active' : ''}`} onClick={onClick} aria-current={active ? 'page' : undefined}>{icon === 'chat' ? <ChatIcon /> : icon === 'org' ? <OrgIcon /> : <DecisionIcon />}<span>{label}</span></button>; }
...
function EmployeeInfoModal({ employee, profiles, onClose }: { employee: Employee; profiles: Record<string, EmployeeProfile>; onClose: () => void }) { const profile = getEmployeeProfile(employee, profiles); const dialogRef = useModalA11y(onClose); return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="employee-info-modal" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="employee-info-title"><header><div className="employee-info-title"><Avatar employee={employee} /><div><span className="eyebrow">EMPLOYEE PROFILE</span><h2 id="employee-info-title">{employee.name}</h2><p>{employee.role} · {employee.department}</p></div></div><button aria-label="关闭" onClick={onClose}>×</button></header><p className="employee-info-summary">{profile.summary}</p><section className="profile-section skills-section"><h3>Skill 能力</h3>{profile.skills.length ? <ul>{profile.skills.map((skill) => <li key={skill.name}>{skill.desc ? <><strong>{skill.name}</strong><span>{skill.desc}</span></> : skill.name}</li>)}</ul> : <p className="skills-empty">暂无，待补充</p>}</section><div className="employee-info-grid"><section className="profile-section strengths"><h3>优点</h3><ul>{profile.strengths.map((item) => <li key={item}>{item}</li>)}</ul></section><section className="profile-section weaknesses"><h3>需要注意</h3><ul>{profile.weaknesses.map((item) => <li key={item}>{item}</li>)}</ul></section><section className="profile-section best-for"><h3>适合做什么</h3><ul>{profile.bestFor.map((item) => <li key={item}>{item}</li>)}</ul></section></div><footer><div className="trait-list">{profile.traits.map((trait) => <span key={trait}>{trait}</span>)}</div><button className="primary-action" onClick={onClose}>返回对话</button></footer></section></div>; }
function DecisionLineModal({ employees, decisionLine, setDecisionLine, onClose }: { employees: Employee[]; decisionLine: DecisionLine; setDecisionLine: (v: DecisionLine) => void; onClose: () => void }) { const [draft, setDraft] = useState<DecisionLine>(decisionLine); const updateNode = (index: number, patch: Partial<DecisionNode>) => setDraft((d) => ({ ...d, nodes: d.nodes.map((n, i) => (i === index ? { ...n, ...patch } : n)) })); const removeNode = (index: number) => setDraft((d) => ({ ...d, nodes: d.nodes.filter((_, i) => i !== index) })); const addNode = () => setDraft((d) => ({ ...d, nodes: [...d.nodes, { employeeId: employees[0]?.id || '', domain: '', keywords: [] }] })); const dialogRef = useModalA11y(onClose); return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="decision-line-modal" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="decision-line-title"><header><div><span className="eyebrow">DECISION LINE</span><h2 id="decision-line-title">决策线配置</h2><p>调度者解析问题后，按关键词分派给对应专家处理</p></div><button aria-label="关闭" onClick={onClose}>×</button></header><div className="dl-field"><label>调度者（负责解析与分派）</label><select value={draft.dispatcherId} onChange={(e) => setDraft((d) => ({ ...d, dispatcherId: e.target.value }))}>{employees.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}</select></div><div className="dl-nodes">{draft.nodes.map((node, index) => <div className="dl-node" key={index}><select value={node.employeeId} onChange={(e) => updateNode(index, { employeeId: e.target.value })}>{employees.map((e) => <option key={e.id} value={e.id}>{e.name}</option>)}</select><input value={node.domain} placeholder="专业领域（如：财务分析 · 风险控制）" onChange={(e) => updateNode(index, { domain: e.target.value })} /><input value={node.keywords.join(',')} placeholder="路由关键词，逗号分隔（如：预算,投资,成本）" onChange={(e) => updateNode(index, { keywords: e.target.value.split(/[,，]/).map((s) => s.trim()).filter(Boolean) })} /><button className="dl-remove" onClick={() => removeNode(index)}>删除</button></div>)}</div><div className="dl-actions"><button className="dl-add" onClick={addNode}>＋ 添加专家</button><button className="primary-action" onClick={() => { setDecisionLine(draft); onClose(); }}>保存配置</button></div></section></div>; }
function NewGroupModal({ employees, onCreate, onClose }: { employees: Employee[]; onCreate: (name: string, memberIds: string[]) => void; onClose: () => void }) { const [name, setName] = useState(''); const [selected, setSelected] = useState<string[]>([]); const toggle = (id: string) => setSelected((cur) => (cur.includes(id) ? cur.filter((x) => x !== id) : [...cur, id])); const dialogRef = useModalA11y(onClose); return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="new-group-modal" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="new-group-title"><header><div><span className="eyebrow">GROUP</span><h2 id="new-group-title">新建群聊</h2><p>选成员，之后可在群里 @ 提问、AI 回复会自动拉人进群</p></div><button aria-label="关闭" onClick={onClose}>×</button></header><div className="dl-field"><label>群名称</label><input value={name} onChange={(e) => setName(e.target.value)} placeholder="如：收购评估小组" /></div><div className="ng-members">{employees.map((e) => <label key={e.id} className="ng-member"><input type="checkbox" checked={selected.includes(e.id)} onChange={() => toggle(e.id)} /><Avatar employee={e} compact /><span>{e.name}</span><em>{e.role}</em></label>)}</div><div className="dl-actions"><button className="dl-add" onClick={onClose}>取消</button><button className="primary-action" disabled={!selected.length} onClick={() => onCreate(name, selected)}>创建群（{selected.length} 人）</button></div></section></div>; }
function GroupInfoModal({ group, employees, onRemove, onAdd, onRename, onClose }: { group: Group; employees: Employee[]; onRemove: (memberId: string) => void; onAdd: (memberId: string) => void; onRename: (groupId: string, name: string) => void; onClose: () => void }) { const [adding, setAdding] = useState(false); const candidates = employees.filter((e) => !group.members.includes(e.id)); const dialogRef = useModalA11y(onClose); return <div className="modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><section className="group-info-modal" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="group-info-name"><header><div><span className="eyebrow">GROUP</span><input className="gi-name-input" id="group-info-name" value={group.name} aria-label="群名称" onChange={(e) => onRename(group.id, e.target.value)} /><p>{group.members.length} 位成员 · AI 回复 @ 的人会自动拉进群</p></div><button aria-label="关闭" onClick={onClose}>×</button></header><div className="gi-members">{group.members.map((id) => { const e = employees.find((x) => x.id === id); return <div className="gi-member" key={id}>{e && <Avatar employee={e} compact />}<span>{e?.name ?? id}</span><em>{e?.role}</em><button className="dl-remove" onClick={() => onRemove(id)}>移出</button></div>; })}</div>{adding && <div className="gi-add">{candidates.length ? candidates.map((e) => <button className="gi-add-item" key={e.id} onClick={() => onAdd(e.id)}><Avatar employee={e} compact /><span>{e.name}</span><em>{e.role}</em><b>＋</b></button>) : <p className="assign-empty">所有员工都已在群里了</p>}</div>}<div className="dl-actions"><button className="dl-add" onClick={() => setAdding((v) => !v)}>{adding ? '收起' : '＋ 添加成员'}</button><button className="primary-action" onClick={onClose}>完成</button></div></section></div>; }
...
function DepartmentMemberModal({ node, currentMemberIds, onClose }: { node: OrgNode; currentMemberIds: string[]; onClose: () => void }) {
  ...
  const dialogRef = useModalA11y(() => { if (!saving) onClose(); });
...
function EmployeeDetailCard({ employee, profile, employeeModels, chatModel, onSetEmployeeModel, onSaveProfile, onOpenChat }: { employee: Employee; profile: EmployeeProfile; employeeModels: Record<string, ChatModel>; chatModel: ChatModel; onSetEmployeeModel: (employeeId: string, model: ChatModel | null) => void; onSaveProfile: (employeeId: string, profile: EmployeeProfile) => Promise<void>; onOpenChat: (id: string) => void; }) {
  ...
  const chips = (key: 'strengths' | 'weaknesses' | 'keywords' | 'notGoodAt', label: string) => (
    <section className="emp-block" key={key}>
      <h3>{label}</h3>
      <div className="emp-chips">{((draft[key] as string[]) || []).map((v, i) => <span className="emp-chip" key={i}>{v}{editing && <button type="button" className="emp-chip-rm" aria-label={`移除${label}`} onClick={() => removeItem(key, i)}>×</button>}</span>)}</div>
      {editing && <div className="emp-add-row"><input className="emp-add-input" placeholder={`添加${label}…`} onKeyDown={(e) => { if (e.key === 'Enter') { addItem(key, e.currentTarget.value); e.currentTarget.value = ''; } }} /><button className="emp-add-btn" aria-label={`添加${label}`} onClick={(e) => { const input = e.currentTarget.previousElementSibling as HTMLInputElement; addItem(key, input.value); input.value = ''; }}>＋</button></div>}
    </section>
  );
  return <div className="emp-detail">
    ...
    <section className="emp-block set">
      ...
      <div className="emp-set-row"><span className="emp-set-label">对话模型</span>{editing ? <select className="emp-input wide" value={employeeModels[employee.id] ?? ''} onChange={(e) => onSetEmployeeModel(employee.id, e.target.value === 'kimi' || e.target.value === 'deepseek' || e.target.value === 'openai' ? e.target.value : null)}><option value="">跟随全局（{modelLabel(chatModel)}）</option><option value="kimi">Kimi</option><option value="deepseek">DS V4</option><option value="openai">GPT</option></select> : <strong className="emp-set-value">{employeeModels[employee.id] ? modelLabel(employeeModels[employee.id]) : `跟随全局（${modelLabel(chatModel)}）`}</strong>}</div>
    </section>
    {chips('strengths', '优点')}
    {chips('weaknesses', '缺点')}
    {chips('keywords', '关键词')}
    ...
    <section className="emp-block skills">
      ...
      {(draft.skills || []).length ? <ul className="emp-skill-list">{(draft.skills || []).map((s, i) => editingSkillIdx === i ? (
        ...
        <li key={i} onClick={() => setExpandedSkill(expandedSkill === i ? null : i)}>
          ...
          {editing && <span className="emp-skill-actions"><button type="button" className="emp-chip-rm" aria-label="编辑 Skill" onClick={(e) => { e.stopPropagation(); startEditSkill(i); }}>✎</button><button type="button" className="emp-chip-rm" aria-label="删除 Skill" onClick={(e) => { e.stopPropagation(); update({ skills: (draft.skills || []).filter((_, j) => j !== i) }); }}>×</button></span>}
        </li>
    ...
    <section className="emp-block career">
      <h3>过往履历</h3>
      {(draft.career || []).length ? <ul className="emp-career-list">{draft.career!.map((c, i) => <li key={i}><span>{c}</span>{editing && <button type="button" className="emp-chip-rm" aria-label="移除履历" onClick={() => removeItem('career', i)}>×</button>}</li>)}</ul> : <p className="emp-empty">暂空，待补充</p>}
      {editing && <div className="emp-add-row"><input className="emp-add-input" placeholder="添加履历（如：2019-2023 XX公司 产品总监）" onKeyDown={(e) => { if (e.key === 'Enter') { addItem('career', e.currentTarget.value); e.currentTarget.value = ''; } }} /><button className="emp-add-btn" aria-label="添加履历" onClick={(e) => { const input = e.currentTarget.previousElementSibling as HTMLInputElement; addItem('career', input.value); input.value = ''; }}>＋</button></div>}
    </section>
  </div>;
}
